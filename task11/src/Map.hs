...
class Map t where
    ...
    fromList list = foldl (\m (k, a) -> insert k a m) empty list
    ...
    insertWith f k a = alter (\x -> maybe (Just a) (\y -> Just (f a y)) x) k
